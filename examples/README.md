...
Eight runnable scripts, each a focused concept, built on `genai_studio.agents`.
...
`01`–`04`, `06`, `07` need only the core; `05` needs the `[datascience]` extra; `08` needs a knowledge base on the gateway (and `[grounding]` to also ground public stats via Data Commons).
